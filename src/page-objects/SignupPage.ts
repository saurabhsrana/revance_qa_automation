...
export class SignupPage {
  ...
  async confirmPhoneNumber() {
    ...
    const alt = this.page
      .locator('[data-button-root="true"].btn-primary, button.btn-primary')
      .filter({
  ...
  private dateOfBirthCalendarPanel() {
    return this.page
      ...
      .filter({ has: this.page.locator('[role="grid"]') });
  }
   ...
   */
  private async selectSvelteDatepickerCalendar(
    dp: Locator,
    yearStr: string,
    monthNum: number,
    dayNum: number,
    ...
    const header = calendar
      .locator("button")
      .filter({ hasText: new RegExp(`\\w+\\s+${year}$`) })
      .first();
    const monthFromHeader = (text: string) =>
      text.replace(/\s*\d{4}\s*$/, "").trim();

    for (let step = 0; step < 12; step++) {
      const headerText = ((await header.textContent()) ?? "").trim();
      if (
        monthFromHeader(headerText).toLowerCase() === targetFull.toLowerCase()
      )
        break;
      ...
      if (currentIdx < 0) {
        throw new Error(
          `Date of birth calendar: unrecognized month header "${headerText}"`,
        );
      }
  ...
  async selectDateOfBirth(isoDate: string) {
    ...
    if (!monthNum || !dayNum)
      throw new Error(`Invalid date parts in: ${isoDate}`);
    ...
    const monthSelect = this.page
      .locator("select#signup-date-of-birth, select.reva-dob-input__month")
      .first();
    const dayInput = this.page
      .getByLabel(/day of birth/i)
      .or(
        this.page.locator(
          "#signup-date-of-birth-day, input.reva-dob-input__day",
        ),
      )
      .first();
    const yearInput = this.page
      ...
      .or(
        this.page.locator(
          "#signup-date-of-birth-year, input.reva-dob-input__year",
        ),
      )
      .first();
    ...
    if (await monthSelect.isVisible().catch(() => false)) {
      await monthSelect.selectOption({ label: monthLabel }).catch(async () => {
        await monthSelect
          .selectOption({ value: String(monthNum) })
          .catch(async () => {
  ...
  async acceptAllConsentCheckboxes() {
    ...
    const noneTreatment = this.page
      ...
      .or(
        this.page.getByRole("checkbox", {
          name: /none of the above|have not received any of these treatments/i,
        }),
      )
      .first();
    ...
    const unlabeled = this.page.locator(
      'input.reva-signup-consent-checkbox[type="checkbox"]',
    );
  ...
  async clickCreateAccount() {
    ...
    try {
      await this.page.waitForURL((url) => !/\/signup\/?$/i.test(url.pathname), {
        timeout: 60_000,
      });
    } catch {
  ...
  async expectDashboardPoints(expectedPoints: string) {
    const points = this.page.getByText(
      new RegExp(`${expectedPoints}\\s*(pts|points)?`, "i"),
    );
