...
test.describe("Revance complete profile", () => {
  for (const data of profileDataSets) {
    test("User completes sign-up, profile questions, and sees the rewards dashboard", async ({
      ...
      await test.step(`I enter the verification code "${data.verificationCode}"`, async () => {
        const otp = verificationCodeFromEnv(data.verificationCode);
        await signupPage.enterVerificationCode(otp);
      });
